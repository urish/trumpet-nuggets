...
(async function () {
  ...
  // Nadav's favorite song
  // (Nadav is the next generatiom GDE!! :-D )
  // Ein Ani - by Shotei HaNevua (Fools of Prophecy)
  
  async function halfChorus() {

    await playNote(A4, 2.8 * whole / 4);
    await delay(0.2 * whole / 4);
    await playNote(A4, whole / 4 );
    await playNote(F5, 2.8 * whole / 4);
    await playNote(F5, whole / 4 );
    await playNote(E5, whole / 4 );
    await playNote(D5, whole / 4 );
    await playNote(C5, whole / 4 );
    await playNote(D5, whole / 4 );
    await playNote(C5, 1.8 * whole / 4 );
    await delay(0.2 * whole / 4);
    ...
    await playNote(A4, whole / 4 );
    await playNote(As4, whole / 4 );
    await playNote(A4, 2 * whole / 4 );
    await playNote(G4, 2 * whole / 4 );
  }
